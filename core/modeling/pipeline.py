...
class ModelingPipelineBuilder:
    ...
    def split_data_stratified(self, X, y, test_size=0.2, random_state=None):
        """
        Splits the data using stratified sampling, while handling classes that have
        fewer than 2 members. Rare classes are temporarily removed, a stratified split is
        performed on the remaining data, and then rare classes are added back into the training set.

        Args:
            X (pd.DataFrame): Feature data.
            y (pd.Series): Target data.
            test_size (float, optional): Proportion for the test split. Defaults to 0.2.
            random_state (int, optional): Random state for reproducibility. Defaults to None.

        Returns:
            X_train, X_test, y_train, y_test: The split data, with rare classes appended to training.
        """
        # Identify classes with fewer than 2 instances
        counts = y.value_counts()
        rare_classes = counts[counts < 2].index.tolist()

        # Split the dataset: remove rows from rare classes
